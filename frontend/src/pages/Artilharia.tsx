...
export default function Artilharia() {
  ...
  const [rachaId, setRachaId] = useState<number | null>(null)
  ...
  useEffect(() => {
    setArtilheiros((prev) => {
      const sorted = [...prev]
      sorted.sort((a, b) =>
        tab === 'gols' ? b.gols - a.gols : b.assistencias - a.assistencias,
      )
      return sorted
    })
  }, [tab])

  async function updateValue(atletaId: number, increment: boolean) {
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">
              Ranking de {tab === 'gols' ? 'Gols' : 'Assistências'}
            </h1>
      ...
      <ul className="flex items-center gap-1 border-b border-gray-800">
        <li><button onClick={() => setTab('gols')} className={tabClass('gols')}>Gols</button></li>
        <li><button onClick={() => setTab('assistencias')} className={tabClass('assistencias')}>Assistências</button></li>
      </ul>
